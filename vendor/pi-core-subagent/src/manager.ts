...
import { READONLY_TOOLS, selectChildTools, WRITE_CAPABLE, WRITE_TOOLS } from "./tool-selection.ts";
import {
	MAX_TASKS,
	type PendingReply,
	type RunDetails,
...
import { createRequestedWebTools } from "./web-tools.ts";
...
export class SubagentManager {
	...
	private async runChild(
		...
		try {
			const selected = selectChildTools({
				write: input.write,
				explicitTools: input.tools,
				fileTools: file?.tools,
			});
			baseTools = selected.baseTools;
			if (selected.toolsNote) task.toolsNote = selected.toolsNote;
		} catch (err) {
			this.updateTask(
				run,
				task,
		...
		const canWrite = baseTools.some((candidate) => WRITE_CAPABLE.some((tool) => tool === candidate));
		...
		try {
			...
			const customTools: ToolDefinition[] = [
				...
				...(await createRequestedWebTools(baseTools)),
			];
