...
- **Agent files respected.** A spawn goal first checks for a case-insensitive exact filename-stem match, then matches a user agent file's `description` (`.agents/agents`, `.claude/agents`, `.pi/agents` — project then home). The selected file's body is the system prompt and its frontmatter `model`/`tools` apply; file `model` is validated against the pi model registry. File wins over inline; no match → on-demand definition.
...
- **Zero parent-context injection.** No catalog, no context hook. Children receive only the task and their selected tools (plus talk tools).
...
Define agents inline per call, or reference a named agent file (see [Agent files](#agent-files)). Model resolution: agent-file `model` (validated against the pi model registry) → explicit `model` → the parent's current model → settings default.
...
A user agent file in an agents directory is first matched by a case-insensitive exact filename stem against the requested agent name. If no exact stem matches, the extension matches `description` frontmatter against the spawn goal (`agent` name + `task`). When matched, the file is **authoritative**: body = system prompt, frontmatter `model`/`tools` apply, inline `prompt`/`model` are ignored — with one exception: explicit per-call `tools`/`write` override the file's tools (the file narrows defaults, it never displaces explicit intent, and it can never widen past the leader's read/write choice). An override is surfaced on the task's notice and summary. Files without a valid `description` are not eligible for either route. No match → the inline on-demand definition stands.
...
Exact filename-stem matches are selected in discovery order (project directories before home). Without an exact match, the file with the highest description-overlap score wins (≥2 shared meaningful tokens). A file `model` is validated against the pi model registry (unknown model fails the task with a catalog message). Files without a `description` frontmatter never match.
